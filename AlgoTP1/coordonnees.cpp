...
double Coordonnees::operator-(const Coordonnees & other) const
{
	...
	return (2.0 * rayon * asin(sqrt(pow(sin(((other.getLatitude() * pi / 180.0) - (this->getLatitude()  * pi / 180.0)) / 2.0), 2.0)  + cos((this->getLatitude() * pi / 180.0)) * cos((other.getLatitude() * pi / 180.0)) * pow(sin(((other.getLongitude() * pi / 180.0) - (this->getLongitude() * pi / 180.0)) / 2.0), 2.0))));
}
